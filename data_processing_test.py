...
def create_csv(csv_file,text_id_mapping_file):
    ...
    text_id_mapping.to_csv(text_id_mapping_file, index=False)
    # 删除original_text_id列
...
if __name__ == "__main__":
    ...
    #     for row in reader:
    #         renamed_files_dict[row[1]] = row[0]
    create_csv(csv_path,'data_csv/res1/text_id_mapping.csv')
    ...
    #     for row in reader:
    #         renamed_files_dict[row[1]] = row[0]
    # create_csv(csv_path,'text_id_mapping_file')
    # create_tsv_file('ImageData_img_text_ys', datasets + "/test_imgs.tsv")
    # create_jsonl_file(csv_path, datasets + "/test_texts.jsonl")
